...
var dj = function dj() {
  if (arguments.length === 0) {
    return dayjs(new Date()).tz();
  }
  var ins = dayjs.apply(void 0, arguments);
  if (arguments.length === 1 && ins.isValid() && dayjs.tz) {
    return dayjs.tz.apply(dayjs, arguments);
  }
  return ins;
};
dj.extend = function (plugin, option) {
  var isInitial = !plugin.$i;
  dayjs.extend(plugin, option);
  if (isInitial) {
    var props = Object.getOwnPropertyNames(dayjs);
    props.forEach(function (prop) {
      if (!Object.hasOwnProperty.call(dj, prop)) {
        var attributes = Object.getOwnPropertyDescriptor(dayjs, prop);
        if (Utils.u(attributes)) {
          return;
        }
        Object.defineProperty(dj, prop, attributes);
      }
    });
  }
  return dj;
};
dj.prototype = dayjs.prototype;
dj.locale = dayjs.locale;
dj.isDayjs = dayjs.isDayjs;
dj.unix = dayjs.unix;
dj.en = dayjs.en;
dj.Ls = dayjs.Ls;
dj.p = dayjs.p;
